...
- Supabase Auth: **solo Google OAuth**. Nessun account email/password: chi accede la prima volta viene creato automaticamente (trigger `handle_new_user`, aggiornato in migration 0006 per leggere i metadati di Google — nome e foto profilo arrivano già pronti da Google, nessuna email di conferma da gestire).
- `app/auth/callback/route.ts` scambia il `code` restituito da Google con una sessione Supabase vera.
