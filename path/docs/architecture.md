...
- Supabase Auth: solo Email/Password. Nessun provider OAuth attivo (Google escluso su richiesta; nessuna predisposizione Apple per ora).
- Alla registrazione, un trigger SQL (`handle_new_user`) crea automaticamente la riga in `profiles`.
- `middleware.ts` aggiorna la sessione ad ogni richiesta e reindirizza a `/login` le rotte protette.
