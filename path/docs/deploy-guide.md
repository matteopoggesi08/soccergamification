...
```bash
unzip coach-diary.zip -d coach-diary
cd coach-diary
git init
git add .
git commit -m "Coach Diary - versione iniziale"
```

### 1.2 Crea il repository su GitHub
1. Vai su [github.com/new](https://github.com/new)
2. Nome repo: `coach-diary` (o quello che preferisci)
...
```bash
git remote add origin https://github.com/TUO-USERNAME/coach-diary.git
git branch -M main
...
### 2.5 Configura il login con Google (obbligatorio — è l'unico metodo di accesso)
...
7. Su Supabase → **Authentication → URL Configuration**: imposta **Site URL** con l'URL reale della tua app (lo aggiornerai al punto 3.4 con quello definitivo di Vercel) e aggiungi lo stesso URL sotto **Redirect URLs**

Senza questo passaggio il pulsante "Continua con Google" dell'app darà errore: è l'unico modo per accedere, non c'è login email/password di riserva.
...
3. Seleziona il repo `coach-diary` → **Import**
...
1. Una volta live, copia l'URL reale assegnato da Vercel (es. `https://coach-diary-tuonome.vercel.app`) — o collega un dominio personalizzato da **Settings → Domains**
...
### 3.5 Primo accesso
1. Apri l'URL dell'app → **Continua con Google**
